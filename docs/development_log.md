...
### Checkpoint 8.5：offset journal 编解码与压缩重启恢复已保存

- `FileRaftPersistence` 的 full/delta codec 改为按 retained boundary 校验 absolute index
  连续性；`commit_index` 与 `lastIndex` 比较，不再误用 vector size。
- delta suffix 的 `expected_index` 改为 `base_index + vector_offset`，兼容 index 50/100
  等非零 boundary 的增量追加与 suffix replacement。
- `RaftNode` 启动加载同样改为 `commit_index <= entries.back().index`，避免压缩日志在
  commit_index=102、vector size=3 时被错误拒绝。
- 新增 offset full image round-trip、offset delta suffix、Snapshot + compacted journal
  服务重启恢复测试。
- 生产路径仍未自动调用 `compactTo` 写盘；下一 checkpoint 将把 Snapshot 发布与 journal
  前缀截断接入 NodeService。
- Debug 96/96 通过（12.77 秒），Release 96/96 通过（12.64 秒），无新增编译告警。

### Checkpoint 8.6：Snapshot 发布与 journal 前缀截断已接入 NodeService

- `RaftNode::compactLogPrefix` 在 `compactTo` 成功后立即 `persistOrThrow`，将压缩后的
  offset journal 写盘。
- `RaftKVService::tryPublishSnapshot` 在 `lastApplied - lastSnapshotIndex >= threshold`
  时先 `FileSnapshotStore::save`，再压缩 Raft 日志；`last_snapshot_index_` 在启动时从
  已有 Snapshot 初始化。
- `NodeServiceConfig::snapshot_entry_threshold`（默认 64）控制触发频率；`runRaft` 每轮在
  `completePending` 之后调用发布逻辑，成功后刷新内存中的 `snapshot_`。
- `FileRaftPersistence::save` 在 boundary 变化时自动回退为 full image，避免 delta 编码
  拒绝前缀截断。
- 新增 3 个测试：阈值门控发布与 journal 压缩、发布后重启恢复、boundary 变化 full
  image 持久化。
- Debug 99/99 通过（12.00 秒），Release 99/99 通过（12.62 秒），无新增编译告警。

### Checkpoint 8.7.1：InstallSnapshot RPC 类型与协议编解码

- `types.h` 新增 `InstallSnapshotRequest` / `InstallSnapshotResponse`，扩展
  `RpcPayload` variant。
- Wire 格式：request 携带 `term`、`leader_id`、`last_included_index/term`、`offset`、
  `done` 与长度前缀 `data` 分块；response 携带 `term` 与 `success`。
- `RaftProtocol` 新增 `kMaximumSnapshotChunkSize`（1 MiB）与 wire type 5/6；
  encode/decode 与帧长上限校验已接入。
- `node_service` 对 InstallSnapshot 消息暂留空桩（8.7.3 接线）。
- 新增 3 个 `raft_protocol_test`：分块 round-trip、超大 chunk 拒绝、response
  round-trip。
- 全量测试 102/102 通过。

### Checkpoint 8.7.2：InstallSnapshot 核心逻辑

- `FileSnapshotStore` 新增 `readBytes`/`fileSize`；`SnapshotFileReceiver` 分块组装并
  原子发布 snapshot 文件。
- `RaftNode` 新增 `peerNeedsSnapshot`、`replaceLogWithSnapshotBoundary`、
  `onInstallSnapshotSuccess`、`observeHigherTerm`。
- `RaftKVService` 实现 `handleInstallSnapshot`、`handleInstallSnapshotResponse`、
  `completeReplication`；Leader 在 `match_index < firstIndex` 时改发 InstallSnapshot。
- `NodeService` 已接线 InstallSnapshot RPC（8.7.3 部分提前完成）。
- 新增 6 个测试：snapshot 分块读写、boundary 压缩、Leader 发送、Follower 安装追上。
- 全量测试 107/107 通过。

### Checkpoint 8.7.4：三节点 InstallSnapshot 集成测试

- `node_service_test` 新增 `LaggingNodeInstallsSnapshotAfterJournalCompaction`：
  先起 node1/2 写入并触发 snapshot，再晚启动 node3，验证 snapshot 安装与重启恢复。
- 修复 `handleInstallSnapshot`：日志 `firstIndex` 领先但 `lastApplied` 未追上时仍安装
  snapshot，避免误判“已覆盖”。
- 修复 `completeReplication`：从 AppendEntries 切换 InstallSnapshot 时重置分块 offset，
  避免离线期间残留进度导致首包 `offset > 0`。
- 修复 `PeerTransport`：断连期间不刷送过期 pending 帧；新 InstallSnapshot 清空 pending；
  pending 满时丢弃最旧帧。
- 修复 `makeInstallSnapshotRpc`：snapshot 元数据不匹配时回退 AppendEntries 而非报错
  中断整批复制。
- 全量测试 108/108 通过。

### 下一步

Phase 8 收尾完成。启动 Phase 9.1：成员配置持久化。

## 2026-07-14 — Phase 9 计划立项

### 目标

在 Phase 8 Snapshot/日志压缩收尾后，实现 Raft 论文 Section 6 的 **joint consensus**
两阶段成员变更，支持在线单步扩容与缩容。

### 计划文档

- 完整分阶段安排见 [phase9_membership_plan.md](phase9_membership_plan.md)。
- 6 个 Checkpoint：9.1 配置持久化 → 9.2 joint 核心 → 9.3 catch-up/Learner →
  9.4 PeerTransport 接线 → 9.5 dkv_admin → 9.6 故障分区门禁。
- 前置依赖：Phase 8.6/8.7（Snapshot 截断 + InstallSnapshot）完成后再进入 9.3 及以后。

### 设计要点

- 成员变更作为 `kConfChange` 日志条目提交，不经过 KV 状态机。
- 过渡态使用 \(C_{old,new}\)，commit 需同时满足 old 与 new 配置的多数派。
- 每次仅允许增删 1 个节点；持久化配置优先于启动参数。
- 非目标：批量变更、自动服务发现、mTLS、分片。

### 下一步

Phase 8.7 完成后启动 Checkpoint 9.1。

## 2026-07-14 — Phase 9.1 成员配置持久化

### 目标

Checkpoint 9.1：成员配置可编码、落盘、重启恢复；为 Phase 9.2 joint consensus 提供持久化基础。

### 变更

- 新增 `cluster_config.h/.cpp`：`ClusterConfiguration`、`ConfChangeEntry` 编解码。
- `EntryType::kConfChange = 2`；`RaftPersistentState` 扩展 `cluster_config` / `joint_config`。
- `FileRaftPersistence` state image 追加 extension v1 tail；config 变化时强制 full image；
  delta 保留 config；修复 delta 解码在读取 command 前校验 payload 的回归。
- `RaftNode` 构造时优先加载持久化配置，否则使用 `bootstrap_cluster`；`persistOrThrow` 写入 config。
- `NodeService::makeRaftConfig` 从 `members` 注入 `bootstrap_cluster`。
- `state_machine` / `raft_protocol` 允许 `kConfChange`（仅推进 `lastApplied`，不修改 KV）。

### 测试

- 全量 **112/112** 通过（新增 4 项 `RaftPersistenceTest` 配置 round-trip / 损坏拒绝 / offset delta 共存）。
- 保持 Phase 8.7.4 三节点 InstallSnapshot 集成测试通过。

### 下一步

进入 **Phase 9.2**：joint consensus 核心（`C_old,new` 提交门禁、ConfChange 日志应用）。

## 2026-07-14 — Phase 9.2 Joint consensus 核心

### 变更

- `RaftNode::proposeConfChange` 支持单步 ADD/REMOVE，并拒绝非 Leader、重复 pending
  或已有 joint 状态下的并发变更。
- Leader 在 joint 阶段按 `C_old` 与 `C_new` 两侧分别计算多数派；投票 quorum 也同步采用
  双多数派，普通配置继续使用原有多数派规则。
- joint ConfChange 提交后自动追加退出 joint 的第二条内部日志条目，避免把两阶段流程
  暴露为两次运维操作；配置提交后动态更新成员资格与复制目标。
- ConfChange 只推进 Raft 应用索引，不写入 KV 状态机；配置仍通过既有持久化 state image
  保存。

### 测试

- 新增确定性 3→4 测试：验证 joint 双多数派、重复变更拒绝、自动退出 joint 与最终配置。
- Debug 全量 **113/113** 通过。

### 下一步

进入 **Phase 9.3**：Learner/catch-up 与新成员在入组前的 Snapshot/日志追赶门禁。

## 2026-07-15 — Phase 9.3 Learner 与 catch-up 核心

### 变更

- `NodeConfig` 增加运行期 Learner 集合及本地 Learner 标志；Learner 接收
  `AppendEntries`/`InstallSnapshot`，但不发起选举、不投票，也不计入任何 commit quorum。
- Leader 为 Learner 初始化独立复制游标，并将其加入复制广播；Learner 追赶期间不会改变
  旧配置 quorum。
- `proposeConfChange(ADD)` 要求目标先登记为 Learner 且 `matchIndex >= commitIndex`，追平前
  拒绝正式入组；追平后移入 joint 配置流程。
- 复用既有 InstallSnapshot 游标路径，压缩日志后的 Learner 可通过 Snapshot 继续追赶。

### 测试

- 新增 Learner 追赶、禁止投票以及追平后允许 ADD 的确定性测试。
- Phase 9.2/9.3 专项测试通过；当前全量测试为 **114/114**。

### 限制

- Learner 集合当前为运行期配置，尚未写入持久化配置模型。
- 新节点 endpoint 的 PeerTransport 动态注册和 NodeService 管理入口留待 Phase 9.4。

### 下一步

进入 **Phase 9.4**：PeerTransport 与 NodeService 的运行期成员/连接更新。

## 2026-07-15 — Phase 9.4 PeerTransport/NodeService 运行期接线

### 变更

- `PeerTransport` 新增线程安全 `addPeer`/`removePeer` 请求接口；调用方只写入更新队列，
  endpoint map、epoll 注册和连接关闭均由 Peer IO 线程执行。
- 移除 peer 时清理 pending frames、注销旧连接；新增或 endpoint 变更时由 IO loop 重新
  建立连接，避免 Raft 线程直接访问连接表。
- `NodeService` 每轮从 stable/joint cluster configuration 计算目标 peer 集合，并同步
  PeerTransport；stable 配置同时刷新 Leader redirect endpoint。
- 线性读 quorum 改为使用 Raft stable/joint 配置计算，动态成员变更期间同时要求两侧多数派。

### 验证

- Debug 全量 **114/114** 通过。
- 已验证既有三节点 TCP、持久重启、Snapshot/InstallSnapshot 和故障重选举测试无回归。

### 限制

- 当前尚未提供 Admin RPC/`dkv_admin`，运行期触发 ConfChange 仍需上层调用 Raft API。
- Learner endpoint 仍需由 NodeService 启动参数或后续 Admin 流程提供；下一阶段实现 Admin
  命令、Leader 重定向和成员发现。

## 2026-07-15 — Phase 9.5 Admin 工具与在线扩缩容

### 变更

- Client wire protocol 升级到 v2，新增结构化 `ADD_NODE`、`REMOVE_NODE`、
  `LIST_MEMBERS` 请求及成员 endpoint 列表响应。
- `NodeService` 在 Raft 线程串行处理单个成员变更；ADD 自动登记动态 Peer endpoint、等待
  Learner 追平，再提交 joint/exit 两阶段日志；REMOVE 在纯新配置提交后才响应成功。
- `dkv_node --learner` 支持新节点按旧稳定配置启动，入组前不选举、不投票；入组后自动
  切换为普通投票成员，被移除节点则自动停止投票。
- 新增 `dkv_admin add-node`、`remove-node`、`show-members`，复用 Leader hint 自动重试。

### 测试

- 新增 Admin 协议及结构化成员发现 round-trip 测试。
- 新增真实 TCP 四节点 `AddsAndRemovesOnlineMember`，覆盖 Learner 启动、3→4、经新节点
  写入、4→3 和成员查询。
- Debug 全量 **117/117** 通过（16.90 秒）。
- Release 全量 **117/117** 通过（17.29 秒），无新增编译告警。

### 下一步

进入 **Phase 9.6**：扩容/缩容期间 Leader 宕机、网络分区与进程恢复门禁。

## 2026-07-15 — Phase 9.6 故障门禁与 Phase 9 完成

### 恢复修复

- 新 Leader 若从持久化状态恢复到已提交 joint 配置，会复用日志中已有退出条目；若旧
  Leader 尚未写出退出条目，则根据稳定/joint 配置差异重建 `C_new` 条目并继续提交。
- 节点启动时以持久化稳定配置恢复本地 voter/learner 身份；已加入 Learner 重启后成为
  voter，被移除节点重启后保持非投票状态。
- joint 配置在日志 append 时立即生效；新成员此时从 Learner 晋升为 joint voter，避免
  原 Leader 宕机后因新侧无法投票造成选举死锁。未提交 joint 被覆盖时同步回滚身份。

### 故障与分区测试

- `cluster_process_test` 覆盖 7 个成员变更真实进程场景：在线扩容、新配置 Leader
  故障、移除节点隔离、全量重启、pre-joint Leader 崩溃、committed-joint Leader
  崩溃恢复、仅旧多数派不能提交。阶段崩溃测试还会原地重启旧 Leader 并检查配置收敛。
- `NewLeaderRecoversCommittedJointExit` 覆盖 joint 阶段 Leader 切换恢复；
  `JointAddRejectsOnlyOldConfigurationMajority` 验证仅旧侧多数派不能提交 ADD。
- `RollsBackOverwrittenUncommittedJointChange` 验证未提交 joint 条目被新 Leader 覆盖时，
  本地临时配置同步回滚，后续不会恢复一个从未提交的成员变更。
- `TornDeltaPreservesJointConfiguration` 验证配置落盘后的 torn delta 恢复不丢失
  stable/joint 配置。
- `scripts/expand_cluster.sh` 已实际运行通过，演示 3→4→3 并验证 KV 数据完整。

### 门禁结果

- Debug 全量 **129/129** 通过（42.07 秒）。
- Release 全量 **129/129** 通过（40.63 秒），无新增编译告警。
- Phase 9 的配置持久化、joint consensus、Learner catch-up、动态 PeerTransport、Admin
  工具及故障门禁全部完成。

### 已知限制

- 每次只能增加或删除一个节点，不支持批量配置变更。
- 不提供跨数据中心自动发现、mTLS/ACL、自动负载均衡或多 Raft Group。
- `LIST_MEMBERS` 返回接收节点的已提交视图，Follower 在 heartbeat 到达前可能短暂落后一版。
- 完整思考总结、测试证据与后续建议见
  [phase9_retrospective.md](phase9_retrospective.md)。

## 2026-07-16 — Phase 10.1 Admin 成员操作幂等化

### 变更

- Client wire protocol 升级到 v3；ADD/REMOVE 强制携带非零 operation id，KV 与成员查询
  禁止夹带该字段。同一请求对象在客户端内部重试期间保持 ID 不变。
- ConfChange 的 joint/exit 两条日志携带相同 operation id；旧日志没有尾部 ID 时仍可解码为
  legacy 操作。Raft 持久化扩展升级为 v2，同时继续读取 v1 配置状态。
- Raft 持久保存 active 与最近一次 completed 成员操作。joint 被覆盖时清理 active；退出
  joint 后原子转为 completed；Leader 切换恢复退出条目时沿用原 ID。
- NodeService 对同 ID、同载荷的 active 请求重新绑定连接，对 completed 请求返回
  `OK_REPLAYED`；同 ID、不同载荷返回 `OPERATION_ID_REUSED`。
- `dkv_admin` 在发送前打印 operation id，并接受可选的末尾 ID，允许响应丢失后跨进程重试。

### 验证

- 新增协议 ID 往返/非法字段、legacy ConfChange、操作状态持久化和 Raft joint/exit 状态转换
  测试。
- 真实 TCP 3→4→3 测试验证 ADD/REMOVE completed 重放与同 ID 载荷冲突。
- 真实进程测试验证 committed-joint Leader 被 SIGKILL 后，同 ID 请求可交给新 Leader 完成；
  全集群重启后仍可重放最近一次 completed ADD。
- Debug 全量 **134/134** 通过（19.19 秒）。
- Release 全量 **134/134** 通过（19.30 秒），无新增编译告警。

### 限制

- 当前只保存最近一次 completed 操作；后续成员变更完成后，更早 operation id 不再保证
  可重放。需要更长审计/重试窗口时，应升级为有界历史并定义清理策略。
