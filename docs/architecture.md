...
`FileRaftPersistence` 接受 offset image：full/delta record 按 boundary 的 absolute
index 校验连续性与 `commit_index` 上界；boundary 变化时自动写 full image 而非 delta。
`RaftNode` 加载使用 `entries.back().index` 而非 vector size。

`RaftKVService::tryPublishSnapshot` 在 applied 条目数超过 `snapshot_entry_threshold`
  时先持久化 `state.snapshot`，再调用 `RaftNode::compactLogPrefix` 截断 journal 前缀。
`NodeService` 在 Raft 事件循环中周期触发；阈值通过 `NodeServiceConfig` 配置（默认 64）。
InstallSnapshot RPC 尚未实现，落后 Follower 仍依赖完整日志复制追赶。
...
9. **Phase 8**：Snapshot、日志前缀压缩与 offset journal（已完成）。
10. **Phase 9**：动态成员变更（joint consensus 在线扩容/缩容，见
    [phase9_membership_plan.md](phase9_membership_plan.md)，已完成）。
...
- Phase 9 按 Raft 论文 Section 6 实现 joint consensus 单步成员变更；ADD 先以 Learner
  追平，进入/退出 joint 的两条配置日志均持久化，选举、提交和线性读在 joint 状态同时
  要求旧、新配置多数派。
- 成员 endpoint 随稳定配置持久化；PeerTransport 在自身 IO 线程动态增删连接。已提交
  joint 配置在 Leader 切换后由新 Leader 重建缺失的退出条目。
- 仍不支持批量成员变更、自动跨数据中心发现、mTLS/ACL 与多 Raft Group。
